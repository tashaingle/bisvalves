  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors duration-500 ${
        onHero ? "bg-transparent" : "border-b border-black/5 bg-white/95 backdrop-blur-md"
      }`}
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`grid h-10 w-10 place-items-center border lg:hidden ${
              onHero ? "border-white/30" : "border-black/10"
            }`}
            aria-label="Menu"
            onClick={() => setOpen((v) => !v)}
          >
            <span className="flex w-4 flex-col gap-1.5">
              <span className={`h-px transition ${onHero ? "bg-white" : "bg-night"} ${open ? "translate-y-[4px] rotate-45" : ""}`} />
              <span className={`h-px transition ${onHero ? "bg-white" : "bg-night"} ${open ? "opacity-0" : ""}`} />
              <span className={`h-px transition ${onHero ? "bg-white" : "bg-night"} ${open ? "-translate-y-[4px] -rotate-45" : ""}`} />
            </span>
          </button>
        </div>
